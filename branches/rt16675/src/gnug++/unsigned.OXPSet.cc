...
Pix unsignedOXPSet::seek(unsigned  item)
{
  int l = p.low();
  int h = p.high();
  while (l <= h)
  {
    int mid = (l + h) / 2;
    int cmp = unsignedCMP(item, p[mid]);
    if (cmp == 0)
...
Pix unsignedOXPSet::add(unsigned  item)
{
  if (count == 0) 
  {
    ++count;
...
void unsignedOXPSet::del(unsigned  item)
{
  int l = p.low();
  int h = p.high();
  while (l <= h)
...
int unsignedOXPSet::operator <= (const unsignedOXPSet& b)
{
...
int unsignedOXPSet::operator == (const unsignedOXPSet& b)
{
  ...
  int i = p.low();
  int j = b.p.low();
  while (n-- > 0) if (!unsignedEQ(p[i++], b.p[j++])) return 0;
  return 1;
}
...
int unsignedOXPSet::OK()
{
  int v = p.OK();
  v &= count == p.length();
  for (int i = p.low(); i < p.high(); ++i) v &= unsignedCMP(p[i], p[i+1]) < 0;
